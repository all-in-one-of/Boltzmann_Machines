...
#weights and biases
rbm_w = tf.placeholder("float", [784, 600])
rbm_vb = tf.placeholder("float", [784])
rbm_hb = tf.placeholder("float", [600])
...
v2 = sample_prob(tf.nn.sigmoid(tf.matmul(h1, tf.transpose(rbm_w)) + rbm_vb))
h2 = tf.nn.sigmoid(tf.matmul(v2, rbm_w) + rbm_hb)

#updating rules of 
w_positive_grad = tf.matmul(tf.transpose(X), h0)
w_negative_grad = tf.matmul(tf.transpose(v2), h2)
update_w = rbm_w + alpha * (w_positive_grad - w_negative_grad) / tf.to_float(tf.shape(X)[0])
update_vb = rbm_vb + alpha * tf.reduce_mean(X - v2, 0)
update_hb = rbm_hb + alpha * tf.reduce_mean(h0 - h2, 0)
...
n_w = 0*np.random.randn(784, 600)
n_vb = np.zeros([784], np.float32)
n_hb = np.zeros([600], np.float32)
o_w = 0*np.random.randn(784, 600)
o_vb = np.zeros([784], np.float32)
o_hb = np.zeros([600], np.float32)
...
for start, end in zip(range(0, len(trX), batchsize), range(batchsize, len(trX), batchsize)):
    ...
    if start % 10000 == 0:
        ...
        image = Image.fromarray(
            tile_raster_images(
                X=sess.run(v_sample, feed_dict={X: trX, rbm_w: n_w, rbm_vb: n_vb, rbm_hb: n_hb}),
